...
Another approach can be called by using the `tree_segment` function. Within the `tree_segment` function there are hyperparameters that require to be passed as a list object and the dataframe that will be used to segment. The output provided is a list containing the tree object (`segment_model`), a summary table (`segment_tree`), the predicted values (`segment_predicted`) and the model input parameters (`model_inputs`).
...
```{r}
# model <- tree_segment(citrus::preprocessed_data, hyperparameters)
rpart.plot(model$segment_model)
``` 
