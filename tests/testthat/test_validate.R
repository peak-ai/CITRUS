...
test_that("Supervised without response variable", {
  expect_error(citrus::validate(output_preprocess, hyperparameters = hyperparameters_tree), regexp = "Columns missing: response")
})

test_that("Correct error when customerid is missing.", {
  expect_error(citrus::validate(preprocess_no_customerid, hyperparameters = hyperparameters_tree), regexp = "Columns missing: customerid")
})

test_that("Throw error when too many categorical levels", {
  expect_error(citrus::validate(preprocess_too_many_categories, hyperparameters = hyperparametershyperparameters_tree), regexp = "Categorical Columns have too many levels: faulty_feature")
})

test_that("Unique customer count", {
  expect_equal(nrow(output_preprocess, hyperparameters = hyperparameters_tree), length(unique(output_preprocess[["customerid"]])))
})
